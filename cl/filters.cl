...
static inline int get_neighbors_avg(uchar const slc_id_layer, uint const v_size, uint const v_id, 
			...
			uint const cel_idx, int const radius)
{
	...
	for (int v_ofs = radius_neg; v_ofs <= radius_pos; v_ofs++) {
		...
		for (int u_ofs = u_z; u_ofs <= u_m; u_ofs++) {
			neighbors_sum += cel_state_3d_safe(slc_id_layer, v_size, v_id, v_ofs, u_size, u_id, u_ofs, cel_states);
			neighbor_count += 1;
		}
	}	
...
__kernel void retina(
	...
	uint const slc_id_layer = get_global_id(0);	// <<<<< TODO: USE A GLOBAL OFFSET
	...
	uint const tar_axn_idz = get_axn_idz(slc_id_layer + cel_base_axn_slc);
	...
	int const neighbors_avg = get_neighbors_avg(slc_id_layer, v_size, v_id, u_size, u_id, cel_states,
		cel_idx, radius);
