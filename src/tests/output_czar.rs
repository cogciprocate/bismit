...
pub fn print_sense_only(cortex: &mut Cortex) {
	...
	if false {
		...
		cortex.cortical_area.ssts.get_mut("iv").expect(EMSG).dens.syns.strengths.print(1 << 0, None, Some((256, 288)), true);
	}
	if false{	
		print!("\nSPINY STELLATE SYNAPSE SOURCE SPINY STELLATE OFFSETS:");
		cortex.cortical_area.ssts.get_mut("iv").expect(EMSG).dens.syns.src_col_xy_offs.print(1 << 0, None, Some((256, 288)), true);
	}

	if false {
		print!("\nPYRAMIDAL DENDRITE SYNAPSE STRENGTHS:");
		cortex.cortical_area.pyrs.get_mut("iii").expect(EMSG).dens.syns.strengths.print(1 << 0, None, Some((256, 319)), true);
	}
}
...
pub fn print_sense_and_print(cortex: &mut Cortex) {
	...
	/* SPINY STELLATE, SPINY STELLATE SYNAPSE, SPINY STELLATE RAW STATES */
			...
			cortex.cortical_area.ssts.get_mut("iv").expect(EMSG).dens.syns.states.print(1 << 3, Some((1, 255)), None, true);
		}*/
	if true {	
		print!("\nSPINY STELLATE SYNAPSE STATES: ");
		cortex.cortical_area.ssts.get_mut("iv").expect(EMSG).dens.syns.states.print(1 << 10, Some((1, 255)), None, true);
	}
	if true {
		print!("\nSPINY STELLATE SYNAPSE SOURCE ROW IDS:");
		cortex.cortical_area.ssts.get_mut("iv").expect(EMSG).dens.syns.src_slice_ids.print(1 << 11, None, None, true);
	}
		if false {
			print!("\nSPINY STELLATE SYNAPSE SOURCE ROW IDS(0 - 1300):");
			cortex.cortical_area.ssts.get_mut("iv").expect(EMSG).dens.syns.src_slice_ids.print(1 << 0, None, Some((0, 1300)), true);
		}
	if true {	
		print!("\nSPINY STELLATE SYNAPSE SOURCE SPINY STELLATE OFFSETS: ");
		cortex.cortical_area.ssts.get_mut("iv").expect(EMSG).dens.syns.src_col_xy_offs.print(1 << 11, None, None, true);
	}
	if true {
		print!("\nSPINY STELLATE SYNAPSE STRENGTHS:");
		cortex.cortical_area.ssts.get_mut("iv").expect(EMSG).dens.syns.strengths.print(1 << 11, None, None, true);
	}
	if false {
		print!("\nSPINY STELLATE PEAK COL IDS: ");
		//cortex.cortical_area.mcols.iinn.spi_ids.print_val_range(1 << 0, Some((0, 255)));
	}
	if false {
		print!("\nSPINY STELLATE PEAK COL STATES: ");
		//cortex.cortical_area.mcols.iinn.states.print_val_range(1 << 0, Some((1, 255)));
	}
	if true {	
		print!("\nSPINY STELLATE DENDRITE STATES: ");
		cortex.cortical_area.ssts.get_mut("iv").expect(EMSG).dens.states.print_val_range(1 << 0, Some((1, 255)));
	}
	if true {	
		print!("\nSPINY STELLATE DENDRITE STATES RAW: ");
		cortex.cortical_area.ssts.get_mut("iv").expect(EMSG).dens.states_raw.print_val_range(1 << 8, Some((1, 255)));
	}
	/*if true {	
	...
	if false {	
		...
		cortex.cortical_area.ssts.get_mut("iv").expect(EMSG).dens.states_raw.print_val_range(1 << 0, Some((1, 255)));
	}
	...
	if true {	
		...
		cortex.cortical_area.pyrs.get_mut("iii").expect(EMSG).dens.syns.states.print(1 << 16, Some((1, 255)), None, true);
	}	

		if false {	
			print!("\nPYRAMIDAL SYNAPSE STATES (all): ");
			cortex.cortical_area.pyrs.get_mut("iii").expect(EMSG).dens.syns.states.print(1 << 0, Some((0, 255)), None, true);
			//print!("\nPYRAMIDAL SYNAPSE STATES (524288 - 524588): ");
			//cortex.cortical_area.pyrs.get_mut("iii").expect(EMSG).dens.syns.states.print(1 << 1, Some((0, 255)), Some((524288, 524588)), true);
		}

	if true {	
		print!("\nPYRAMIDAL SYNAPSE SOURCE ROW IDS: ");
		cortex.cortical_area.pyrs.get_mut("iii").expect(EMSG).dens.syns.src_slice_ids.print(1 << 14, None, None, true);
	}

		if false {
			print!("\nPYRAMIDAL SYNAPSE SOURCE ROW IDS(0 - 1300):");
			cortex.cortical_area.pyrs.get_mut("iii").expect(EMSG).dens.syns.src_slice_ids.print(1 << 1, None, Some((0, 1300)), true);
		}
	...
	if true {	
		...
		cortex.cortical_area.pyrs.get_mut("iii").expect(EMSG).dens.states_raw.print_val_range(1 << 8, Some((1, 255)));
	}
	if false {
	...
	if true {
		...
		cortex.cortical_area.pyrs.get_mut("iii").expect(EMSG).soma_mut().print_val_range(1 << 0, Some((1, 255)));
	}
