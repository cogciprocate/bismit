...
fn cycle_random_pyrs() {
    ...
    area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().set_all_to_zero(true);
    ...
    area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().syns_mut().src_slc_ids().cmd().fill(zeroed_slc_id, None).enq().unwrap();
    area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().syns_mut().src_slc_ids().default_queue().unwrap().finish().unwrap();
    ...
    println!("\nDEBUG INFO: \n{mt}{}, \n{mt}synapse dims: {:?}",
        area.area_map(), area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().lyr_dims(), mt = cmn::MT);
...
fn _test_rand_cel(area: &mut CorticalArea, zeroed_slc_id: u8, src_slc_id: u8, iter: usize) {
    ...
    for tft_id in area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().tft_id_range() {
        // And for each dendrite:
        for den_id_tft in area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().den_id_range_celtft(tft_id) {
            let tft_den_idz = area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().den_idzs_by_tft()[tft_id];
            let tft_dims = area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().tft_dims_by_tft()[tft_id].clone();

            let den_coords = DenCoords::new(cel_coords.clone(), tft_id, tft_den_idz,
                tft_dims.clone(), den_id_tft);

            let tft_syn_idz = area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().syn_idzs_by_tft()[tft_id];
            ...
            // area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().syns_mut().src_slc_ids.set_range_to(src_slc_id,
            //     den_syn_range.clone()).unwrap();

            let fill_size = den_syn_range.end - den_syn_range.start;

            area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().syns_mut().src_slc_ids().default_queue().unwrap().finish().unwrap();
            area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().syns_mut().src_slc_ids().cmd()
            ...
            // Cycle entire cell:
            area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().cycle_solo();
            area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().cycle_solo();
            area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().cycle_solo();
            ...
            let den_idx = area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().den_idx(&cel_coords, tft_den_idz, &tft_dims, den_id_tft);

            let mut den_state = vec![0];
            let mut cel_state = vec![0];
            area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().states().cmd().read(&mut den_state).offset(den_idx as usize).enq().unwrap();
            area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().soma().cmd().read(&mut cel_state).offset(cel_coords.idx() as usize).enq().unwrap();
            ...
            if den_state == 0 || cel_state == 0 {
                ...
                print!("\n");
                area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().print_all();
                area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().print_all();
                area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().print_all();
                area.print_aux();
            ...
            util::eval_others(&area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().states(), den_idx as usize, 0);
            util::eval_others(&area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().soma(), cel_coords.idx() as usize, 0);

            //=============================================================================
            //================================= CLEAN UP ==================================
            //=============================================================================

            // area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().syns_mut().src_slc_ids.set_range_to(zeroed_slc_id, den_syn_range).unwrap();
...
fn cycle_random_dens() {
    ...
    // area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().syns_mut().set_all_to_zero();
    area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().set_all_to_zero(true);
    ...
    area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().src_slc_ids().default_queue().unwrap().finish().unwrap();
    area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().src_slc_ids().cmd().fill(zeroed_slc_id, None).enq().unwrap();
    area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().src_slc_ids().default_queue().unwrap().finish().unwrap();
    ...
    // ////// DEBUG: 2016-Dec-24
    //     // Print src_slc_ids:
    //     area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().print_src_slc_ids();

    //     let lyr_syn_count = area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().states().len();
    //     area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().states().cmd().fill(99, Some(5)).offset(lyr_syn_count - 5).enq().unwrap();
    //     area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().states().cmd().fill(99, Some(5)).enq().unwrap();
    //     area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().print_all();

    //     // Re-zero all syn states:
    //     area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().states().cmd().fill(0, None).enq().unwrap();
    // //////
    ...
    for test_iter in 0..DENS_TEST_ITERATIONS {
        ...
        let cel_coords = if test_iter == 0 {
            area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().last_cel_coords()
        } else {
            area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().rand_cel_coords()
        };

        let den_coords = area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().rand_den_coords(cel_coords.clone());

        let tft_syn_idz = area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().syn_idzs_by_tft()[den_coords.tft_id];
        ...
        area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().syns().src_slc_ids().cmd()
        ...
        for den_idx in den_coords.den_idx_range_celtft() {
            // area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().states.enqueue_read(&mut result[..], den_idx as usize); REMOVE
            // let den_state = result[0]; REMOVE
            // let den_state = area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().states.read_idx_direct(den_idx as usize);

            let den_state = util::read_idx_direct(den_idx as usize, area.layer_test(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens().states());
        ...
        // area.layer_test_mut(testbed::PRIMARY_TEMPORAL_PYR_LAYER_NAME).unwrap().dens_mut().syns_mut().src_slc_ids.set_range_to(zeroed_slc_id, cel_syn_range).unwrap();
