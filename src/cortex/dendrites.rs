...
impl Dendrites {
            ...
            ) -> CmnResult<Dendrites>
    {
        ...
        let energies = Buffer::builder().queue(ocl_pq.queue().clone()).dims(den_count_ttl).fill_val(0, None::<()>).build()?;
        let activities = Buffer::builder().queue(ocl_pq.queue().clone()).dims(den_count_ttl).fill_val(0, None::<()>).build()?;
