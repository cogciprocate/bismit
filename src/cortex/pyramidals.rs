...
impl PyramidalLayer {
    pub fn new<S: Into<String>>(
        ...
        //     DendriteKind::Distal, area_map, axons, ocl_pq, settings.disable_pyrs, exe_graph)?;
        ...
        let pyr_cycle_kernel = ocl_pq.create_kernel(kern_name)?
            .gws(SpatialDims::One(cel_count))
            .arg_buf(tfts.tft_best_den_ids())
            .arg_buf(tfts.tft_best_den_states_raw())
            .arg_buf(tfts.tft_best_den_states())
        ...
        for tft_id in 0..tft_count {
            cycle_cmd_srcs.push(CorticalBuffer::data_soma_tft(tfts.tft_best_den_ids(), layer_addr, tft_id));
            cycle_cmd_srcs.push(CorticalBuffer::data_soma_tft(tfts.tft_best_den_states_raw(), layer_addr, tft_id));
            cycle_cmd_srcs.push(CorticalBuffer::data_soma_tft(tfts.tft_best_den_states(), layer_addr, tft_id));
        }
        ...
        Ok(PyramidalLayer {
            ...
            // tft_ltp_kernels: tft_ltp_kernels,
            // tft_cycle_kernels: tft_cycle_kernels,
