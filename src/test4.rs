...
pub fn readback_test<T: Int + Clone + Default>(
			cortex: &Cortex,
			test_source: &Vec<T>,
	...
	println!("	{} Iterations complete in: {} sec.",READBACK_TEST_ITERATIONS , 0);
...
pub fn print_vec_info<T: Int + Clone + Default>(my_vec: &Vec<T>, info: &str) {
	let mut total = 0;
