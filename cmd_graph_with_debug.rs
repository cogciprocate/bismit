...
impl CommandDetails {
    ...
    pub fn write_buffer_id(&self) -> usize {
        match *self {
            CommandDetails::Write { target } => target,
            _ => panic!("CommandDetails::write_buffer_id: Not a write command."),
        }
    }

    pub fn read_buffer_id(&self) -> usize {
        match *self {
            CommandDetails::Read{ source } => source,
            _ => panic!("CommandDetails::read_buffer_id: Not a read command."),
        }
    }
}
